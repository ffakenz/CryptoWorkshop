...
contract("Gaia Test", function (accounts) {
    ...
    it("reject to create event if not owner", async () => {
        ...
        await truffleAssert.reverts(
            this.gaiaStore.createEvent(eventId, 311221, ticketPrice, [], {
                from: other,
            }),
            CALLER_IS_NOT_THE_OWNER
        );
    ...
    it("reject unknown buyer", async () => {
        ...
        await this.gaiaStore.createEvent(
            eventId,
            311221,
            ticketPrice,
            [ticketId],
    ...
    it("sell ticket to whitelisted customer", async () => {
        ...
        assert.equal(
            await web3.eth.getBalance(this.gaiaStore.address),
            ticketPrice
        );
    });
});
